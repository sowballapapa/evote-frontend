...
import {UserCollectorService} from '../../../../../services/comptes/user-collector.service';

@Component({
  selector: 'app-admin-acc-collectors-create',
  standalone: true,
    imports: [
        NgIf,
        NoAccUsersPaginateTableComponent
    ],
  templateUrl: './admin-acc-collectors-create.component.html',
  styleUrl: './admin-acc-collectors-create.component.css'
})
export class AdminAccCollectorsCreateComponent {

  @ViewChild(NoAccUsersPaginateTableComponent, {static: true}) paginator!: NoAccUsersPaginateTableComponent;
  private _unsubscribeAll: Subject<any>;

  constructor(private collectorUserService: UserCollectorService) {
    this._unsubscribeAll = new Subject();
  }

  allUsers:any
  fetchTitle = "Liste des collecteurs sans compte"
  isLoading = false;
  lastPage = 1
  currentPage = 1
  pageSize = [1]
  ...
  fetch(page:number){
    ...
    this.collectorUserService.getNoAccountsUser(page)
