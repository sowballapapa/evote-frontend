...
@Component({
  selector: 'app-admin-acc-electors-create',
  standalone: true,
  imports: [
    NgIf,
    NoAccUsersPaginateTableComponent
  ],
  templateUrl: './admin-acc-electors-create.component.html',
  styleUrl: './admin-acc-electors-create.component.css'
})
export class AdminAccElectorsCreateComponent {

  @ViewChild(NoAccUsersPaginateTableComponent, {static: true}) paginator!: NoAccUsersPaginateTableComponent;
  private _unsubscribeAll: Subject<any>;

  constructor(private electorUserService: UserElectorService) {
    this._unsubscribeAll = new Subject();
  }

  allUsers:any
  fetchTitle = "Liste des électeurs sans compte"
  isLoading = false;
  lastPage = 1
  currentPage = 1
  pageSize = [1]
  ...
  fetch(page:number){
    ...
    this.electorUserService.getNoAccountsUser(page)
